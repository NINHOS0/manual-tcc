import { galleryItem, linkItem, textItem } from "@/interfaces/contentApi"
...
export default function HomeMainItem({ item }: HomeMainItemProps) {
  return (
    <>
      ...
      : item.type === "link"
      ? <Link display={"inline-flex"} alignItems={"center"} gap={"1"} color={"blue.300"} w={'fit-content'} fontSize={item.fontSize} fontWeight={item.fontWeight} href={`${item.url}`}><ExternalLinkIcon boxSize={'3.5'} />{item.value}</Link>
      : item.type === "gallery"
      ? (
